...
async function requestWebshot(
  ...
  const response = await fetch(`${PUSH_API}/webshot?apiKey=${serviceKey}`, {
    method: 'POST',
    headers: {
...
async function getWebshotStatus(id: string, serviceKey: string) {
  const response = await fetch(`${PUSH_API}/webshot/${id}?apiKey=${serviceKey}`);
  const data = await response.json();
  return data.webshot.status;
}

async function getWebshot(id: string, serviceKey: string) {
  const response = await fetch(`${PUSH_API}/webshot/${id}/result?apiKey=${serviceKey}`);
