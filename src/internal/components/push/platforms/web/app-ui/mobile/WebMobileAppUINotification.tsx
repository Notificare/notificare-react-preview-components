...
export function WebMobileAppUINotification({ notification, onError }: WebMobileAppUIProps) {
  const application = useApplication();
  const { googleMapsAPIKey } = useOptions();
  const intl = useIntl();

  useEffect(
    function handleInvalidPreviews() {
      if (!googleMapsAPIKey && notification.type === 're.notifica.notification.Map') {
        onError(
          intl.formatMessage({
    ...
    [notification.type, googleMapsAPIKey, intl, onError],
  );
        ...
        {(() => {
          switch (notification.type) {
            ...
            case 're.notifica.notification.Map':
              return <MapRichContent notification={notification} width={'100%'} height={'400px'} />;
